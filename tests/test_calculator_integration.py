...
def test_perform_operation_multiply_stub(monkeypatch):
    # Stub utils.multiply if missing so branch "3" is covered
...
def test_perform_operation_divide_stub(monkeypatch):
    # Stub utils.divide if missing so branch "4" is covered
...
def test_interactive_invalid_choice_then_success(monkeypatch, capsys):
    # First perform raises ValueError; second returns 42.0
    calls = {"n": 0}
...
def test_interactive_zero_division_then_success(monkeypatch, capsys):
    # First perform raises ZeroDivisionError; second returns 3.0
    calls = {"n": 0}
    ...
    inputs = iter([
        "4", "1", "0",      # first loop -> ZeroDivisionError
